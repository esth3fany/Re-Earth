import * as THREE from 'three';
            ...
            // Importa o GLTFLoader para carregar modelos 3D
            ...
            const MODULE_TYPES = {
                'Residência': { 
                    ...
                    modelScale: 0.03, // Escala para ajustar o modelo
                    area: 50,
                ...
                'Painel Solar': {
                    color: 0x0e7490,
                    icon: 'sun',
                    modelUrl: 'https://raw.githubusercontent.com/esth3fany/Re-Earth/6a97c91f56bc56ffda2972a287692e3e5d3697fc/home_2.glb',
                    modelScale: 1.5,
                    area: 10,
                ...
                'Composteira': { 
                    ...
                    icon: 'recycle',
                    modelUrl: 'https://raw.githubusercontent.com/esth3fany/Re-Earth/6a97c91f56bc56ffda2972a287692e3e5d3697fc/home_2.glb',
                    modelScale: 0.1, // Adicionada uma escala padrão
                    area: 6,
            ...
            // --- Loaders ---
            // Gerenciador global para rastrear todos os assets carregando
            const loadingManager = new THREE.LoadingManager();
            let gltfLoader;
            const placeholderMaterialCache = {};

            // --- Elementos DOM (do App Principal) ---
            let canvasContainer;
            ...
            // --- INICIALIZAÇÃO ---
            // A função init() agora é chamada pelo botão "Jogar"
            function init() {
                // Pega os elementos do DOM *depois* que o app é revelado
                ...
                // Instancia o loader com o gerenciador global
                gltfLoader = new GLTFLoader(loadingManager);

                setupLights();
                setupControls();
                loadAssets(); // Esta função *usa* o loadingManager
                setupUI();
            ...
            function loadAssets() {
                // Passa o loadingManager para o TextureLoader
                ...
                // O loadingManager.onLoad só será disparado *depois* que esta textura carregar
            ...
            function onCanvasClick(event) {
                ...
                if (intersects.length > 0) {
                    ...
                    while (objectToSelect.parent && !objectToSelect.userData.name) {
            ...
            // --- UI ---
            // Esta função agora configura os botões *dentro* do app
            function setupUI() {
                ...
                // O gerenciador de loading (para o spinner *do modelo*) é configurado na função setupMainMenu()
                // porque o loadingManager global agora controla a transição principal.
                // Vamos ajustar a lógica de carregamento do *modelo*
                loadingManager.onStart = (url, itemsLoaded, itemsTotal) => {
                    // Este manager agora controla a tela de loading principal E os modelos
                    // A tela principal já está visível se estamos carregando
                };
                
                // O loadingManager.onLoad é configurado em setupMainMenu() para trocar da tela de loading para o app
                ...
                Object.entries(MODULE_TYPES).forEach(([name, config]) => {
                    ...
                    // Classes atualizadas para o novo design
                    button.className = 'module-button flex flex-col items-center justify-center p-2 bg-green-100 rounded-2xl text-xs font-bold text-green-800 transition-all duration-200 transform hover:scale-110 hover:shadow-xl border-4 border-transparent cursor-grab hover:border-yellow-500';
                    
                    // Os ícones SVG foram atualizados para ter a cor 'text-gray-700'
            ...
            // --- LÓGICA PRINCIPAL (Carregamento de Modelo 3D) ---
            ...
            function addModule(name, config, position, rotation, scale) {
                if (config.modelUrl) {
                    ...
                    // O gltfLoader usa o loadingManager global
                    gltfLoader.load(
                        config.modelUrl, 
                        (gltf) => {
            ...
            function updateDashboardUI(totals, balances) {
                ...
                const updateBar = (barEl, balance, production) => {
                    ...
                    // As cores do gradiente já estão no HTML, só precisamos lidar com o 'déficit'
            ...
            function populateChecklist() {
                ...
                CHECKLIST_ITEMS.forEach(name => {
                    ...
                    li.className = 'text-red-600'; // Cor de 'não completo'
            ...
            function loadStateFromFile(event) {
                ...
                reader.onload = (e) => {
                    try {
                        ...
                        // Não use alert()
                        console.log("Erro: Não foi possível carregar o arquivo de design.");
                    }
                };
            ...
            function setupMainMenu() {
                ...
                playBtn.addEventListener('click', () => {
                    ...
                    loadingOverlay.classList.add('flex'); // Usar flex para centralizar
                    
                    // --- CORREÇÃO AQUI ---
                    // 1. Torne o contêiner do aplicativo visível no DOM (removendo 'hidden')
                    //    para que ele tenha dimensões (clientWidth/clientHeight).
                    appContainer.classList.remove('hidden');
                    appContainer.classList.add('flex'); // Garante que o layout flex seja aplicado
                    
                    // 2. Mantenha-o invisível usando opacidade
                    //    (O 'transition-opacity' no HTML fará isso suavemente mais tarde)
                    appContainer.style.opacity = '0';
                    // --- FIM DA CORREÇÃO ---

                    // Dá um pequeno delay para a UI atualizar antes de começar o 'init'
                    setTimeout(() => {
                        init(); // Agora o init() encontrará um container com tamanho
                    }, 100); 
                });
                ...
                // O loadingManager global cuidará da transição da tela de carregamento para o app
                loadingManager.onLoad = () => {
                    loadingOverlay.classList.add('hidden');
                    loadingOverlay.classList.remove('flex');
                    
                    // --- CORREÇÃO AQUI ---
                    // Apenas mude a opacidade para 1.
                    // As classes 'hidden' e 'flex' já foram tratadas no clique do 'play-btn'.
                ...
                loadingManager.onError = (url) => {
                    ...
                    // Não use alert()
                    loadingOverlay.classList.add('hidden');
                    mainMenu.classList.remove('hidden'); // Volta ao menu principal
                };
            }
            
            // --- Iniciar a aplicação ---
            // Não chamamos init() diretamente, chamamos o setup do menu.
            setupMainMenu();
